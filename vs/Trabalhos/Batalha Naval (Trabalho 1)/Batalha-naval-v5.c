...
int main()
{
    ...
    char linha_ataque='0';  //Variavel que coleta a linha que o usuario deseja atacar
    ...
    int acertos=0;          //Variavel auxiliar que conta quantos navios o usuario acertou
    ...
    printf("\nInsira a linha que deseja atacar(de A-E): "); //Captura a linha que o usuario deseja atacar
        ...
        linha_ataque= toupper(linha_ataque);
            ...
            if(linha_ataque>'E'||linha_ataque<'A'){ //Verifica se digitou uma linha valida
    ...
    printf("\nInsira a coluna que deseja atacar(de 1-5): "); //Captura a coluna que o usuario deseja atacar
            ...
            if(coluna_ataque>5||coluna_ataque<1){  //Verifica se digitou uma coluna valida
                printf("Voce digitou uma coluna inexistente.\n");
                return 5;
            }


    for(i=0;i<o;i++){ //Verifica se o usuario acertou o disparo em algum navio
        if(linha_ataque==n_l[i]&&coluna_ataque==n_c[i]){
            acertos++;
        }
    }
    if(acertos==1){  //Imprime o oceano com um W na casa do navio acertado
        printf("      1   2   3   4   5\n");
        for (i=0;i<=o;i++){
            printf("    +---+---+---+---+---+\n");
    ...
    else{  //Imprime o oceano com um * na casa sem navio que o usuario atacou
        printf("      1   2   3   4   5\n");
        for (i=0;i<=o;i++){
            printf("    +---+---+---+---+---+\n");
